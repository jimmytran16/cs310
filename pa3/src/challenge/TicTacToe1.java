...
//	}
	public boolean equals(Object rhs) {
		if (!(rhs instanceof TicTacToe1))
			return false;

		for (int i = 0; i < 3; i++)
			for (int j = 0; j < 3; j++)
				if (board[i][j] != ((TicTacToe1) rhs).board[i][j])
					return false;
		return true;
	}
	...
	// copy board (deep copy, i.e. copy array content) to new TicTacToe1 object
	// This action parallels the creation of a Position object in old code
	public TicTacToe1 clone() {
		TicTacToe1 clone = new TicTacToe1();  // creates new array
		for (int i = 0; i < 3; i++)
